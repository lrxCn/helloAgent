...
**产品名称：** helloAgent — 云端与本地协同的智能 AI 助手

**核心愿景：** 打造一个既能利用云端强大算力，又能保障本地隐私安全的工业级多模态 AI Agent。

**核心特性：**

1. **云端与本地协同**：支持云端高性能 LLM 与本地轻量化模型（如 Ollama/Llama）协同部署，并预留量化、蒸馏与微调演进方案。
2. **插件化能力 (MCP/Skill)**：支持 MCP 协议与自定义 Skill 扩展，可无缝对接业务接口，让 AI 具备行动力。
3. **分层记忆系统**：深度集成短、中、长期记忆架构，支持超长周期的精准多轮对话。
4. **安全知识库**：支持知识库的分权分域管理，确保底层数据与隐私的绝对安全。
5. **高可扩展架构**：借鉴开源前沿思想，从 0 到 1 构建，核心模块均可深度定制与二次开发。

**项目定位：** 从实战出发，逐步演进为高可扩展、可定制的工业级 Agent 框架。
...
1.  **第一步：持久化存储** ✅ 已实现。引入 `SQLChatMessageHistory`，将对话记录存入本地 SQLite，实现跨会话记忆。
2.  **第二步：滑动窗口与摘要生成** ✅ 已实现。实现监控逻辑，当消息数达阈值时，自动提取老旧消息生成 Summary 并更新 Context 模板。
3.  **第三步：记忆向量化沉淀** ✅ 已实现。将废弃的原始消息归档至向量数据库，配合 `Condense Question` 链实现“记忆召回”。

### 7.6 本地化与云端协同 (Local-Cloud Hybrid & Model Optimization)

- **现状**：目前主要依赖云端 API (SiliconFlow/DeepSeek)。
- **演进**：支持 Ollama / LocalAI 等本地推理引擎。针对不同场景选择模型：
  1. **本地轻量级**：处理隐私数据、简单的文本清洗与初级摘要。
  2. **云端高性能**：处理复杂推理、长文档理解与高精度重排。
  3. **模型优化**：提供针对业务数据的微调（Fine-tuning）方案，以及针对本地部署的量化（Quantization）与蒸馏（Distillation）指引。

### 7.7 插件化能力与 MCP 协议 (MCP & Skill Extension)

- **现状**：Agent 仅具备“查文档”和“聊天”能力，缺乏对外部系统的行动力。
- **演进**：集成 Model Context Protocol (MCP)。
  - **Skill 扩展**：支持动态加载 Skill 模块（如查询天气、搜索实时新闻、操作数据库、调用企业内部 API）。
  - **能力闭环**：通过 Function Calling 让 LLM 自主决定何时调用何种工具，实现从“知”到“行”的跨越。
